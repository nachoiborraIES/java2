...
!!! quote "Exercise 1"
    ...
    Go back to Exercise 1 of [this document](05c.html). Replace the video game array in `main` method with a generic `ArrayList`. Then, add some video games to the list (either `VideoGame` or `PCVideoGame` objects), explore and show the list with a `for` and ask the user to:
